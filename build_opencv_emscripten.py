...
def determine_emcc_version(emscripten_dir):
    ret = subprocess.check_output(
        [os.path.join(emscripten_dir, "emcc"), "--version"])
    m = re.match(
        r'^emcc.*(\d+\.\d+\.\d+)', ret.decode('utf-8'), flags=re.IGNORECASE)
    return m.group(1)
...
def determine_opencv_version(version_hpp_path):
    ...
    with open(version_hpp_path, "rt") as f:
        data = f.read()
        major = re.search(r'^#define\W+CV_VERSION_MAJOR\W+(\d+)$', data,
                          re.MULTILINE).group(1)
        minor = re.search(r'^#define\W+CV_VERSION_MINOR\W+(\d+)$', data,
                          re.MULTILINE).group(1)
        revision = re.search(r'^#define\W+CV_VERSION_REVISION\W+(\d+)$', data,
                             re.MULTILINE).group(1)
        version_status = re.search(
            r'^#define\W+CV_VERSION_STATUS\W+"([^"]*)"$', data,
            re.MULTILINE).group(1)
...
class Builder:
    def __init__(self, options):
        ...
        self.install_dir = check_dir(options.install_dir)
        self.emscripten_dir = check_dir(options.emscripten_dir)
        self.opencv_version = determine_opencv_version(
            os.path.join(self.opencv_dir, "modules", "core", "include",
                         "opencv2", "core", "version.hpp"))
        self.emcc_version = determine_emcc_version(self.emscripten_dir)

    def get_toolchain_file(self):
        return os.path.join(self.emscripten_dir, "cmake", "Modules",
                            "Platform", "Emscripten.cmake")

    def clean_build_dir(self):
        for d in [
                "CMakeCache.txt", "CMakeFiles/", "bin/", "libs/", "lib/",
                "modules"
        ]:
            rm_one(d)
    ...
    def get_cmake_cmd(self):
        ...
        cmd = [
            ...
            "-DCPU_DISPATCH=''",
            "-DCV_TRACE=OFF",
            "-DCMAKE_INSTALL_PREFIX='%s'" % self.options.install_dir,
    ...
    def build_opencv(self):
        execute(["make", "-j", str(multiprocessing.cpu_count())])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Build OpenCV with Emscripten')
    ...
    parser.add_argument(
        '--install_dir',
        default="install/",
        help="Path to installation folder to use for build")
    #    parser.add_argument('--build_wasm', action="store_true", help="Build OpenCV.js in WebAssembly format")
    parser.add_argument(
        '--build_test', action="store_true", help="Build tests")
    parser.add_argument(
        '--build_doc', action="store_true", help="Build documentation")
    parser.add_argument(
        '--clean_build_dir', action="store_true", help="Clean build dir")
    parser.add_argument(
        '--skip_config', action="store_true", help="Skip cmake config")
    parser.add_argument(
    ...
    if not args.skip_config:
        ...
        #           target = "wasm"
        log.info("=====")
        log.info("===== Config OpenCV build for %s" % target)
        log.info("=====")
        builder.config()
    ...
    log.info("=====")
    log.info("===== Building OpenCV with emscripten -> asm.js")
    log.info("=====")
    builder.build_opencv()

    log.info("=====")
    log.info("===== Build finished. Installing")
    log.info("=====")

    builder.install_opencv()
